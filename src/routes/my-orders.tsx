...
import { Package, ShoppingBag, Clock, CheckCircle2, Truck, XCircle, ChevronDown, ChevronUp, ArrowRight } from "lucide-react";
...
const statusConfig: Record<string, { label: string; icon: any; cls: string }> = {
  pending: { label: "بانتظار التحقق", icon: Clock, cls: "bg-amber-500/15 text-amber-700 border-amber-300" },
  confirmed: { label: "تم تأكيد الطلب", icon: CheckCircle2, cls: "bg-emerald-500/15 text-emerald-700 border-emerald-300" },
  processing: { label: "قيد التحضير والتجهيز", icon: Package, cls: "bg-purple-500/15 text-purple-700 border-purple-300" },
  shipped: { label: "جاري التوصيل", icon: Truck, cls: "bg-blue-500/15 text-blue-700 border-blue-300" },
  delivered: { label: "تم التسليم بنجاح ✓", icon: CheckCircle2, cls: "bg-primary/15 text-primary border-primary/30" },
  ...
  if (orders.length === 0) {
    return (
      <div className="mx-auto max-w-md px-4 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">لم تقم بأي طلب حتى الآن، ابدأ رحلتك مع محمصة خصب!</p>
        <Link
          to="/products"
          search={{}}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-lg font-black text-primary tracking-wider">{order.code}</span>
                  ...
                  <div className="text-end">
                    <div className="text-lg font-black text-primary">
                      {currency === 'YER' ? formatPrice(order.total_yer ?? 0) : `${order.total_sar ?? 0} ريال سعودي`}
                    </div>
                          ...
                          <div key={step} className="flex flex-1 items-center gap-1">
                            ...
                            {idx === statusSteps.length - 1 && (
                              <div className={`h-4 w-4 rounded-full border-2 flex-shrink-0 ${
                                isActive ? 'border-primary bg-primary' : isDone ? 'border-primary bg-primary/20' : 'border-muted'
                              }`} />
                            )}
                          </div>
                        );
              ...
              {isExpanded && (
                <div className="border-t bg-muted/20 px-5 py-4 space-y-3">
                  ...
                  <div className="border-t pt-3 flex items-center justify-between font-extrabold text-primary">
                    <span>الإجمالي</span>
                    <span>{currency === 'YER' ? formatPrice(order.total_yer ?? 0) : `${order.total_sar ?? 0} ريال سعودي`}</span>
                  </div>
                </div>
              )}
            </div>
